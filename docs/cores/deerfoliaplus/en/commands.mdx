...
description: List DeerFoliaPlus fake-player, posture, utility, and administrator commands with their permission nodes.
icon: Terminal
sidebar:
...
Commands use Bukkit permissions. Posture permissions default to TRUE; quick-workbench permissions default to OP; confirm the effective defaults of other utility commands with your permission plugin and startup log. A command appearing in completion does not make it safe for every player.

Test with a non-admin account before exposing a command. Grant individual nodes rather than the broad deerfoliaplus.command parent.
...
Empty-hand right-click chair interaction also requires chair-interaction. With require-side-signs enabled, the chair-row sign condition must also pass.
...
Time, weather, target-player state, and item-copy commands can affect a survival server's economy or gameplay. Keep them with trusted administrators by default.
...
inspect is an administrative capability and should be isolated from ordinary players. more can create unintended item quantities, so keep it with a management group.
...
These interfaces can open without a nearby block, so permission design must account for the server economy.
...
Action and dynamic-config subcommands have additional nodes:
...
Use completion for action names and arguments:
...
Fake-player data includes world position, inventory, and custom state. Confirm whether data must be retained before using remove or save.
