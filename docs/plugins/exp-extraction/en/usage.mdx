...
2. Run `/extract 100`, or run `/extract` and let the plugin calculate the amount from your current experience.
...
On success, the plugin drops an experience bottle near the player and removes the stored amount from the player's total experience. The default per-bottle limit is `2000` points; when a requested amount is higher, the plugin warns and clamps this extraction to `2000`.

The plugin does not create a bottle when the argument is not an integer, is negative, exceeds the player's available experience, or the player has no experience to extract.
...
Each successful extraction consumes one empty glass bottle and creates one experience bottle. The default amount is `100`; when less experience is available, the actual amount is reduced to the player's remaining experience.
...
- The current enchanting-table path has no separate permission check, so revoking `expextraction.extract` does not automatically disable table extraction.
