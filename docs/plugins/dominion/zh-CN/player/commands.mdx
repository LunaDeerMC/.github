---
title: 玩家文档 · 命令参考
description: 当前 Dominion 4.9.1-release 面向玩家的命令、参数和使用示例。
icon: Terminal
...
自动创建使用管理员的 `auto-create-radius`。`resize` 的方向名是完整的 `north`、`east`、`south`、`west`、`up`、`down`，不是旧文档中可能出现的单字母写法。
...
`set_env` 修改领地环境规则；`set_guest` 修改访客权限。当前命令补全只会显示 `flags.yml` 中 `enable: true` 的 flag。成员和组权限使用前一节中的 `member_set_pri`、`group_set_flag`。
...
1. 先检查命令拼写和参数顺序；
2. 检查自己是否在目标领地内，或是否拥有该领地的管理权限；
3. 检查 flag 是否在 `flags.yml` 启用；
4. 检查 `limitations/` 是否限制了数量、尺寸、子领地或传送；
5. 仍有问题时，记录完整错误消息、服务端版本、插件版本和触发命令，不要只截取最后一行。
